...
public class ScriptureBox 
{
    ...
    private Dictionary<int, Scripture> _scriptures = new Dictionary<int, Scripture>()
    {
        ...
        {5, new Scripture(new Reference("Doctrine and Covenants", 1, 38), @"What I the Lord have spoken, I have spoken, and I excuse not myself; and though the heavens and the earth pass away, my word shall not pass away, but shall all be fulfilled, whether by mine own voice or by the voice of my eservants, it is the same.")}
    };
    ...
    {
        int _newScripterIndex = _scriptures.Count + 1; //the last 

        //Inserting the new Scripture

        _scriptures.Add(_newScripterIndex,newScripture);
    ...
    public void printScriptures()
    {
        ...
        foreach(KeyValuePair<int, Scripture> i in _scriptures) //I've learned that KeyValuePair we can pass with the all dictionary
        {
            int key = i.Key;
            Scripture _value = i.Value;

            Console.WriteLine(_value.GetDisplayText());//Printing the scriptures.
            Console.WriteLine();
